...
# Let's import the data generated by debug_cashflow and debug_twr
# Just a simple re-implementation of the exact balances
import json
import subprocess

# Run debug_twr2.ts with JSON output
code = """
...
with open("generate_json.ts", "w") as f:
    f.write(code)

import subprocess
import json

out = subprocess.check_output(["npx", "ts-node", "generate_json.ts"]).decode('utf-8')
data = json.loads(out)

df = pd.DataFrame(data)
df['day'] = pd.to_datetime(df['day'])

# 1. Standard TWR (End of Day CFs)
cumulative = 1.0
last_nav = 0
for idx, row in df.iterrows():
    if last_nav == 0:
        ...
        ret = (row.nav - row.cf) / last_nav - 1
        # To avoid division by zero or huge jumps, let's just do it cleanly
        if last_nav > 0:
            cumulative *= (1 + ret)
        last_nav = row.nav
print(f"Standard TWR: {cumulative - 1:.2%}")

# 2. Simple Simple Return
profit = df.iloc[-1].nav - df.cf.sum()
print(f"Simple Absolute Return: {profit / df.cf.sum() if df.cf.sum() != 0 else 0:.2%}")

# 3. Modified Dietz
# Sum of weights * cash flows
total_days = (df.iloc[-1].day - df.iloc[0].day).days
weighted_cf = 0
for idx, row in df.iterrows():
    if row.cf != 0:
        weight = (total_days - (row.day - df.iloc[0].day).days) / total_days
        weighted_cf += row.cf * weight
print(f"Modified Dietz Return: {profit / weighted_cf if weighted_cf > 0 else 0:.2%}")

# 4. Try TWR with START OF DAY CF?
# If CF is start of day, it earns return on that day.
# ret = (nav) / (last_nav + cf) - 1
cumulative2 = 1.0
last_nav2 = 0
for idx, row in df.iterrows():
    base = last_nav2 + row.cf
    if base > 0:
        ret = (row.nav) / base - 1
        cumulative2 *= (1 + ret)
    last_nav2 = row.nav
print(f"Mid/Start of Day CF TWR: {cumulative2 - 1:.2%}")

# Annualized
years = total_days / 365.25
print(f"Annualized TWR: {(cumulative**(1/years)) - 1:.2%}")
