...
import yahooFinance from "https://esm.sh/yahoo-finance2@2.3.10"; // Using a specific version known to work well in Deno/Edge
  ...
  try {
    ...
    if (mode === "fundamentals") {
      ...
      try {
        ...
        // yahoo-finance2 handles batching and crumb auth automatically
        ...
        // Mark missing tickers
        uniqueTickers.forEach(t => {
          if (!results[t]) results[t] = { error: "Not found or data unavailable" };
        });
      ...
      return new Response(JSON.stringify({ results, debugMode: mode, source: "yahoo-finance2" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
