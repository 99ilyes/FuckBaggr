...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (type === "Opération") {
      ...
      transactions.push({ date, type: txType, symbol: formatSymbol(symbolRaw), quantity: parsed.qty, price: finalPrice, amount, currency, exchangeRate });
  ...
  for (const day of days) {
    ...
    for (const tx of dayTxs) {
      ...
      if (tx.type === "DEPOSIT" || tx.type === "WITHDRAWAL") {
        ...
        cashBalance += tx.amount; positions.set(tx.symbol, (positions.get(tx.symbol) || 0) + tx.quantity);
  ...
  console.log(`Final TWR with threshold > 5: ${(cumulativeTwr*100).toFixed(2)}%`);
}
main();
