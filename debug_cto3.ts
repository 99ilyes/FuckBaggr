...
async function main() {
    ...
    let start = new Date("2025-02-15");
    let end = new Date("2025-04-15");
    const days: string[] = [];
    ...
    console.log("\n=== TWR & NAV TRACE (March 2025 Focus) ===");
    for (const day of days) {
        ...
        if (lastNav === 0) {
            ...
            const isTargetRange = day >= "2025-02-15" && day <= "2025-04-15";
            if (day >= "2025-02-25" && day <= "2025-03-31" && (dayTxs.length > 0 || returnPct !== 0)) {
                ...
                if (dayTxs.length > 0) {
                    dayTxs.forEach(t => console.log(`  -> ${t.type} ${t.amount || ""} ${t.symbol || ""}`));
                }
            }
            lastNav = nav;
        }
